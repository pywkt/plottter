...
Map layers benefit significantly from the **Join paths at junctions** step in the
Optimize pipeline. A road network is a graph where many paths share interior
vertices (T-junctions and 4-way crossings). Plain *Merge nearby endpoints* handles
co-incident endpoints but cannot chain across interior junctions; *Join* splits
paths at T-junctions and then traces Eulerian walks across the connectivity graph,
producing much longer uninterrupted strokes.

### Benchmark results

Measured on a synthetic 10 × 10 city-block grid (220 road segments, ±0.04 mm
independent per-node jitter — representative of OSM projection noise at typical
plotter scales):

| Stage | Pen lifts | Pen-up travel | Reduction vs raw |
|-------|-----------|---------------|-----------------|
| Raw (no processing) | 220 | 2302 mm | — |
| After Merge only (0.05 mm threshold) | 54 | 1918 mm | 75.5 % |
| After Merge + Join (0.10 mm threshold) | 24 | 480 mm | 89.1 % |

Join's extra reduction **beyond Merge alone**: 30 / 54 = **55.6 %** — well above
the ≥ 30 % target that justifies auto-enabling it.

Pen-up travel drops from ~1918 mm to ~480 mm (75 % less air travel) once Join is
applied on top of Merge.

Reproduce these numbers with:

```bash
python scripts/benchmark_join.py
```

### Auto-enabled for map layers

Because the improvement is so consistent on road networks, **Join is automatically
enabled** whenever you run *Optimize* on a layer generated by the Map generator —
even if your saved Optimize settings have Join turned off. Non-map layers are never
affected; your saved preference is honoured exactly as-is.

The detection is based on the layer's `generator_info` metadata
(`_generator_name == "Map"`), set automatically when the Map generator produces the
layer.

### Recommended Optimize settings for maps

| Setting | Recommended value | Notes |
|---------|------------------|-------|
| Remove duplicate segments | off | Map generator doesn't emit duplicates |
| Merge nearby endpoints | **on**, 0.05 mm | Tight threshold avoids bridging unconnected roads |
| Join paths at junctions | **on** (auto) | Auto-enabled for map layers; leave at 0.10 mm |
| 2-opt improvement | on | Good travel-distance reduction |
| Or-opt improvement | on | Extra refinement with little overhead |
